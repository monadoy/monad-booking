...
// Sets the language for the names of Months and Days. See the src/lang directory for supported
// languages
...
// Thursday in a year, you could write:  time.makeOrdinalTime(0, 0, 0, JANUARY, FIRST, THURSDAY,
// year)
...
#define LAST_READ \
	(int32_t)0x7FFFFFFE  // (So yes, ezTime might malfunction two seconds before everything else...)
...
void events();
time_t makeOrdinalTime(const uint8_t hour, const uint8_t minute, const uint8_t second,
...
// The following macros are used in calculating alarms and assume the clock is set to a date later
// than Jan 1 1971 Always set the correct time before settting alarms
