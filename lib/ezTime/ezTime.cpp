...
// The include below includes the dayStr, dayShortStr, monthStr and monthShortStr from the
// appropriate language file in the /src/lang subdirectory.
...
#ifdef EZTIME_NETWORK_ENABLE
		if (_ntp_interval)
			updateNTP();  // Start the cycle of updateNTP running and then setting an event for its
			              // next run
#endif
		_initialised = true;
	}
...
time_t makeTime(const uint8_t hour, const uint8_t minute, const uint8_t second, const uint8_t day,
	...
	tm.Day = day;
	tm.Month = month;
	if (year > 68) {  // time_t cannot reach beyond 68 + 1970 anyway, so if bigger user means actual
		              // years
		tm.Year = year - 1970;
	} else {
		tm.Year = year;
...
// makeOrdinalTime allows you to resolve "second thursday in September in 2018" into a number of
// seconds since 1970 (Very useful for the timezone calculations that ezTime does internally) If
// ordinal is 0 or 5 it is taken to mean "the last $wday in $month"
time_t makeOrdinalTime(const uint8_t hour, const uint8_t minute, uint8_t const second,
                       uint8_t ordinal, const uint8_t wday, const uint8_t month, uint16_t year) {
	if (year <= 68)
...
bool queryNTP(const String server, time_t& t, unsigned long& measured_at) {
	...
	// also checking that all timestamps are non-zero and receive timestamp seconds are <= transmit
	// timestamp seconds
	if ((buffer[1] < 1) or (buffer[1] > 15) or (reftsSec == 0) or (rcvtsSec == 0)
	    or (rcvtsSec > secsSince1900)) {
	...
	measured_at
	    = done - ((done - started) / 2)
	      - ms;  // Assume symmetric network latency and return when we think the whole second was.
...
time_t Timezone::tzTime(time_t t, ezLocalOrUTC_t local_or_utc, String& tzname, bool& is_dst,
                        int16_t& offset) {
	if (t == TIME_NOW) {
		t = nowUTC();
	...
	int16_t std_offset
	    = (offset_hr < 0) ? offset_hr * 60 - offset_min : offset_hr * 60 + offset_min;
	...
	if (!start_month) {
		...
		// in local time
		time_t dst_start = ezt::makeOrdinalTime(start_time_hr, start_time_min, 0, start_week,
		                                        start_dow + 1, start_month, tm.Year + 1970);
		time_t dst_end = ezt::makeOrdinalTime(end_time_hr, end_time_min, 0, end_week, end_dow + 1,
		                                      end_month, tm.Year + 1970);
...
bool Timezone::setCache() {
	...
	if (readCache(olson, posix, months_since_jan_2018)) {
		...
		if ((year() - 2018) * 12 + month(LAST_READ) - months_since_jan_2018
		    > MAX_CACHE_AGE_MONTHS) {
...
bool Timezone::writeCache(String& str) {
	...
	// Followed by payload, compressed. Every 4 bytes to three by encoding only 6 bits, ASCII
	// all-caps
	str.toUpperCase();
	uint8_t store = 0;
...
int16_t Timezone::getOffset(time_t t /*= TIME_NOW */,
                            const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	String tzname;
	bool is_dst;
	int16_t offset;
...
uint8_t Timezone::setEvent(void (*function)(), const uint8_t hr, const uint8_t min,
                           const uint8_t sec, const uint8_t day, const uint8_t mnth, uint16_t yr) {
	time_t t = ezt::makeTime(hr, min, sec, day, mnth, yr);
	return setEvent(function, t);
}

uint8_t Timezone::setEvent(void (*function)(), time_t t /* = TIME_NOW */,
                           const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
...
void Timezone::setTime(const uint8_t hr, const uint8_t min, const uint8_t sec, const uint8_t day,
                       const uint8_t mnth, uint16_t yr) {
	tmElements_t tm;
...
String Timezone::dateTime(time_t t, const ezLocalOrUTC_t local_or_utc,
	...
	for (uint8_t n = 0; n < format.length(); n++) {
		...
		if (escape_char) {
			...
			switch (c) {
				case '\\':  // Escape character, ignore this one, and let next through as literal
				            // character
				...
				case 'l':  // (lowercase L) A full textual representation of the day of the week
					...
					break;
				case 'N':  // ISO-8601 numeric representation of the day of the week. ( 1 = Monday,
				           // 7 = Sunday )
					tmpint8 = tm.Wday - 1;
					if (tmpint8 == 0)
						tmpint8 = 7;
					out += String(tmpint8);
					break;
				case 'S':  // English ordinal suffix for the day of the month, 2 characters (st, nd,
				           // rd, th)
					switch (tm.Day) {
						case 1:
						case 21:
				...
				case 'e':  // Timezone identifier (Olson)
					...
					break;
#endif
				case 'O':  // Difference to Greenwich time (GMT) in hours and minutes written
				           // together (+0200)
				case 'P':  // Difference to Greenwich time (GMT) in hours and minutes written with
				           // colon (+02:00)
					o = offset;
					...
					out += ezt::zeropad(o % 60, 2);
					break;
				case 'Z':  // Timezone offset in seconds. West of UTC is negative, east of UTC is
				           // positive.
				...
				case 'W':
					out += ezt::zeropad(
					    weekISO(t), 2);  // ISO-8601 week number of year, weeks starting on Monday
					break;
				case 'X':
					out += String(yearISO(t));  // ISO-8601 year-week notation year, see
...
String Timezone::militaryTZ(time_t t /*= TIME_NOW */,
                            const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	t = tzTime(t, local_or_utc);
	int16_t o = getOffset(t);
	if (o % 60)
		return "?";  // If it's not a whole hour from UTC, it's not a timezone with a military
		             // letter code
	o = o / 60;
	if (o > 0)
...
uint16_t Timezone::dayOfYear(time_t t /*= TIME_NOW */,
                             const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	t = tzTime(t, local_or_utc);
...
uint8_t Timezone::hourFormat12(time_t t /*= TIME_NOW */,
                               const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	t = tzTime(t, local_or_utc);
	uint8_t h = t / 3600 % 12;
	if (h)
...
#define startISOyear(year...) \
	ezt::makeOrdinalTime(0, 0, 0, FIRST, THURSDAY, JANUARY, year) - 3UL * SECS_PER_DAY;
uint8_t Timezone::weekISO(time_t t /*= TIME_NOW */,
                          const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	t = tzTime(t, local_or_utc);
	int16_t yr = year(t);
...
uint16_t Timezone::yearISO(time_t t /*= TIME_NOW */,
                           const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	t = tzTime(t, local_or_utc);
	int16_t yr = year(t);
	time_t this_year = startISOyear(yr);
...
uint8_t second(time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->second(t, local_or_utc));
}
uint8_t setEvent(void (*function)(), const uint8_t hr, const uint8_t min, const uint8_t sec,
...
uint8_t setEvent(void (*function)(), time_t t /* = TIME_NOW */,
                 const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->setEvent(function, t, local_or_utc));
}
void setTime(const uint8_t hr, const uint8_t min, const uint8_t sec, const uint8_t day,
             const uint8_t month, const uint16_t yr) {
	defaultTZ->setTime(hr, min, sec, day, month, yr);
}
void setTime(time_t t) { defaultTZ->setTime(t); }
uint8_t weekISO(time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->weekISO(t, local_or_utc));
}
uint8_t weekday(time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->weekday(t, local_or_utc));
}
uint16_t year(time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->year(t, local_or_utc));
}
uint16_t yearISO(time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */) {
	return (defaultTZ->yearISO(t, local_or_utc));
}
}  // namespace ezt
